...
from . import utils
from .api import header as h
from .api import types as t
from .api.api import BassError
from .api.headprivate import BassConfigOptions as cfgo
...
class BassConfig:
    ...
    def _getconf(self, option:str): 
        try:
            return self.lib.GetConfig(option)
    ...
    def _getconfptr(self, option:str):
        try:
            return self.lib.GetConfigPtr(option)
        except BassError as e:
            if e.code == 20:
                return None
    ...
    @property
    def algorithm3d(self) -> h.Bass3DAlorithmsOptions:  
        ''' The positioning algorithm for 3D channels '''
        return self._getconf(cfgo.ALGORITHM3D)
    @algorithm3d.setter
    def algorithm3d(self, algo: h.Bass3DAlorithmsOptions): 
    ...
    def android_aaudio(self) -> bool: 
        ...
        return None if result == t.MINUSONE else result
    ...
    @property
    def ios_session(self) -> h.BassIOSSessionFlags: 
        '''[iOS] Audio session configuration'''
        #TODO check for -1
        return self._getconf(cfgo.IOS_SESSION)
    @ios_session.setter
    def ios_session(self, config: h.BassIOSSessionFlags): 
    ...
    @property
    def net_playlist(self) -> h.BassNetPlaylistOptions: 
        ''' Enable process URLs in PSL and M3U playlists'''
        return self._getconf(cfgo.NET_PLAYLIST)
    @net_playlist.setter
    def net_playlist(self, playlist:h.BassNetPlaylistOptions): 
    ...
    @property
    def noramp(self) -> h.BassNorampOptions:
        ''' Playback ramping setting'''
        return self._getconf(cfgo.NORAMP)
    @noramp.setter
    def noramp(self, noramp:h.BassNorampOptions): 
    ...
    @property
    def src(self) -> h.BassSamplerateConversionOptions|int: 
        ''' Default samplerate conversion quality'''
        return self._getconf(cfgo.SRC)
    @src.setter
    def src(self, quality:h.BassSamplerateConversionOptions|int):
        self._setconf(cfgo.SRC, quality)

    @property
    def src_sample(self) -> h.BassSamplerateConversionOptions|int: 
        ''' Default samplerate conversion quality for sample channels'''
        return self._getconf(cfgo.SRC_SAMPLE)
    @src_sample.setter
    def src_sample(self, quality:h.BassSamplerateConversionOptions|int): 
